...
function formatMeta(role: string, location: string) {
  const parts = role
    .split("/")
    .map((part) => part.trim())
    .filter(Boolean);
  return [...parts, location].join(" · ");
}
...
export function ShippedCaseStudy() {
  ...
  const isPaused =
    userInteracted || hoverPaused || prefersReducedMotion;
      ...
      <div className="mx-auto max-w-[1400px]">
        <div>
          ...
          <p className="mt-6 max-w-xl text-base leading-relaxed text-kasi-grey">
            Every business has different goals. Here are a few of the websites
            and digital solutions we&apos;ve built for our clients.
          </p>
        </div>
          ...
          <div className="mt-12 flex flex-wrap gap-2">
            {shippedWork.map((w, i) => (
              <button
                ...
                className={cn(
                  "min-h-11 border px-5 py-2.5 text-sm tracking-wide transition",
                  i === active
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <p className="font-mono text-[11px] tracking-[0.14em] text-kasi-grey">
                {formatMeta(work.role, work.location)}
              </p>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex min-h-12 items-center border border-kasi-green bg-kasi-green px-5 py-3 text-sm tracking-wide text-kasi-black transition hover:bg-transparent hover:text-kasi-green"
                >
                  Visit Website →
                </a>
                <Link
                  ...
                  onClick={() =>
                    track("start_project_click", {
                      source: `featured_work_${work.slug}`,
                    })
                  }
                  className="inline-flex min-h-12 items-center border border-kasi-border px-5 py-3 text-sm tracking-wide text-kasi-ivory transition hover:border-kasi-green hover:text-kasi-green"
                >
            ...
            >
              <BrowserFrame url={shippedWorkDisplayHost(work)}>
                <a
                  ...
                  aria-label={`Visit ${work.name} website`}
                >
                  {work.cover ? (
                    <SafeImage
                      src={work.cover}
